 ...
 if (CurrentNode=(struct ExecNode *) CopyExecNode(node)) {
  ...
  if (gl=CreateGadgetList(gdata,GADGETS)) {
   ...
   if (w=OpenWindowTags(NULL,WA_Left,        parent->LeftEdge,
    ...
    /* Set local variables; sub-window uses its own port, main loop dispatches via SubWindowPort/SubWindowHandler */
    w->UserData=(BYTE *) HandleExecEditWindowIDCMP;
    ModifyIDCMP(w,WINDOW_IDCMP);
    CurrentWindow=w;
    SubWindowPort=w->UserPort;
    SubWindowHandler=HandleExecEditWindowIDCMP;
...
void UpdateExecEditWindow(void *data)
{
 ...
 /* Restore sub-window port so main loop dispatches to exec window again */
 SubWindowPort=w->UserPort;
 SubWindowHandler=HandleExecEditWindowIDCMP;
